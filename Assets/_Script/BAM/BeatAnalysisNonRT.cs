...
//	}
	...
	{		
		...
		if (_audio.clip.channels < 2) {
			...
			Debug.LogError ("channel<2"+samples.Length );
		} else {
	...
	#if UNITY_WEBGL 
	/// <summary>
	/// bool The process with update.
	/// </summary>
	bool ProcessWithUpdate=false;
	/// <summary>
	/// 拆分出的数据数量，相当于帧数
	/// </summary>
	int NumOfFrame;
	...
	/// <summary>
	/// 拆分数据，time即采样间隔时间，单位毫秒
	/// </summary>
	/// <param name="time">Time.</param>
	/// <param name="samplesize">Samplesize.</param>
	public void DataProcessWithUpdatePrev(int time=100,int samplesize=128){
	...
	public void DataProcessWithUpdate(int time=100,int indexU=0,int samplesize=128 )
	{	
		int SamplePerFrame = time;
		...
		int startindex = firstDataInd+indexU * SamplePerFrame;//开始采样的编号
	...
	{		//将完整音乐细分，计算每段波长wavelengths
		...
		Debug.LogError ("CalcWavelength  num="+numSubdivide);
	...
	/// <summary>
	/// Checks the beat in clip.单频段检测节拍
	/// </summary>
	/// <param name="indBand">Ind band.</param>
